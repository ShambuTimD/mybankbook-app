...
import { route } from "ziggy-js"; // Ensure route is imported if not globally available
...
const FailedPage = () => {
    ...
    // ✅ helper functions (copied from ThankYouPage)
    ...
    useEffect(() => {
        ...
        const bookingData = sessionStorage.getItem("korpheal_booking_data");
        ...
        const parsedForm = bookingData ? JSON.parse(bookingData) : null;
        ...
        if (storedApptDate) {
            setAppointmentDateText(toDdMonYyyy(storedApptDate));
        } else if (storedRequestDate) {
            setAppointmentDateText(toDdMonYyyy(storedRequestDate));
        }
        ...
        const officeFromSession = sessionStorage.getItem(
        ...
        setCompanyData(
            parsedCompany
                ? { ...parsedCompany, display_center: displayCenter }
                : null
        );
    ...
    const handleGoBack = () => {
        ...
        // We still set the step so the Index page knows where to put the user,
        // but we don't put it in the URL anymore.
        sessionStorage.setItem("korpheal_resume_step", "choose-mode");

        // ✅ CHANGED: Removed the second argument { resume: true }
        window.location.assign(route("brandselfappointment.index"));
    };
            ...
            <div className="min-h-[80vh] flex items-center justify-center padding_faild px-6 py-14">
                <div className="bg-white shadow-xl faild_pa rounded-xl p-10 max-w-2xl w-full text-center">
                    ...
                    <p className="text-base md:text-lg text-gray-700 mb-8">
                        Something went wrong with your booking. Please find the
                        details below for your reference. Please try again. A
                        support team member will assist you soon.
                    </p>
                    ...
                    {sessionData && (
                        <div className="space-y-4 mb-8 text-left text-gray-800">
                            ...
                            {appointmentDateText && (
                                <p>
                                    <strong>Appointment Request Date:</strong>{" "}
                                    {appointmentDateText}
                                </p>
                            )}
                            ...
                            {(employees.length > 0 || totalDependents > 0) && (
                                <p>
                                    ...
                                    {employees.length + totalDependents}{" "}
                                    (Employees: {employees.length}, Dependents:{" "}
                                    {totalDependents})
                                </p>
                            )}
                            ...
                            {sessionData.dos && (
                                <p>
                                    ...
                                    {toReadableDateTime(sessionData.dos)}
                                </p>
                            )}
                        </div>
                    )}
